...
import { FaFileAlt, FaHashtag, FaLink } from "react-icons/fa";
...
const EssayResourceForm: React.FC<{
  resourceData?: Resource;
  onSuccess?: () => void;
}> = ({ resourceData, onSuccess }) => {
  const [form, setForm] = useState<Resource>(
    resourceData || { url: "", content: "", essay_id: "" },
  );
  ...
  return (
    ...
    >
      <label
        htmlFor="essay-resource-essay-id"
        className="flex items-center gap-2 text-cyan-700 font-bold"
      >
        <FaHashtag /> Essay ID
      </label>
      <input
        id="essay-resource-essay-id"
        type="text"
        value={form.essay_id}
