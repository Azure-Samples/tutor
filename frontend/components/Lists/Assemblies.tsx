...
const AssembliesList: React.FC = () => {
  ...
  return (
    <div className="w-full p-0">
                ...
                {assemblies.map((a) => (
                  ...
                  >
                    <td className="py-4 px-2 text-center">
                      {a.service === "essays" ? (
                        <span className="inline-block px-3 py-1 rounded-full text-xs font-bold bg-gradient-to-br from-yellow-300 to-pink-400 text-white shadow">
                          Essays
                        </span>
                      ) : (
                        <span className="inline-block px-3 py-1 rounded-full text-xs font-bold bg-gradient-to-br from-green-300 to-lime-400 text-white shadow">
                          Questions
                        </span>
                      )}
                    </td>
            ...
            {assemblies.length === 0 && (
              <div className="flex flex-col items-center justify-center py-16">
                ...
                <p className="text-green-700 dark:text-green-200 mb-4">
                  Click <span className="font-bold">New Assembly</span> to configure your first AI
                  evaluator swarm.
                </p>
              </div>
            )}
      ...
      <FormsModal
        open={showDeleteModal && !showCreateModal && !showDetailsModal}
      ...
      <FormsModal
        open={showDetailsModal && !showCreateModal && !showDeleteModal}
        onClose={closeAllModals}
        ...
        {selectedAssembly && (
          <div className="p-4 flex flex-col gap-4">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="font-bold text-lg text-blue-700 dark:text-cyan-200">
                {selectedAssembly.topic_name}
              </span>
              {selectedAssembly.essay_id && (
                <span className="text-sm text-green-700 dark:text-green-300">
                  Essay: {selectedAssembly.essay_id}
                </span>
              )}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {selectedAssembly.agents.map((agent) => (
                <div
                  key={
                    agent.agent_id ??
                    `${agent.deployment}-${agent.role ?? agent.dimension ?? agent.name ?? "agent"}`
                  }
                  className="rounded-2xl border-2 border-purple-200 dark:border-purple-800 bg-purple-50 dark:bg-purple-900 p-4 shadow"
                >
                  <p className="font-bold text-purple-700 dark:text-purple-200 text-lg mb-1">
                    {agent.role ?? agent.dimension ?? agent.name ?? "Agent"}
                  </p>
                  <p className="text-sm text-blue-700 dark:text-blue-300 mb-1">
                    Deployment: <span className="font-semibold">{agent.deployment}</span>
                  </p>
                  {agent.agent_id && (
                    <p className="text-sm text-cyan-700 dark:text-cyan-300 mb-1">
                      ID: <span className="font-mono text-xs">{agent.agent_id}</span>
                    </p>
                  )}
                  {agent.dimension && (
                    <p className="text-sm text-green-700 dark:text-green-300 mb-1">
                      Dimension: <span className="font-semibold">{agent.dimension}</span>
                    </p>
                  )}
                </div>
              ))}
