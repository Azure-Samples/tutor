...
const port = Number.parseInt(process.env.FUNCTIONS_CUSTOMHANDLER_PORT || "3000", 10);
...
app.prepare().then(() => {
  const server = createServer(serverOptions, async (req, res) => {
    try {
      ...
      if (userAgent?.includes("BlueRidgeAlwaysOn")) {
        return res.end();
      }
      ...
      if (msOriginalUrl) {
        // always use msOriginalUrl if it is defined, as the req.url can have trimmed paths from MSHA etc.
