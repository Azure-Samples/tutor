...
- `SERVICE_BUS_LEARNER_RECORD_TOPIC`
...
Provisioning automatically assigns `Cosmos DB Built-in Data Contributor` to each backend Container App identity at both the Cosmos account scope for SDK metadata reads and the shared database scope for data access.
