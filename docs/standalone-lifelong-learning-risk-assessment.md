...
- Content ingestion, OCR, RAG, and pedagogical rules in [docs/adr/006-pedagogical-content-ocr-rag.md](./adr/006-pedagogical-content-ocr-rag.md)
- Zero-trust target architecture in [docs/security.md](./security.md) and [docs/adr/004-security-zero-trust.md](./adr/004-security-zero-trust.md)
...
- Frontend MSAL and protected routes are still planned work in [docs/adr/005-frontend-modernization-workspace-ux.md](./adr/005-frontend-modernization-workspace-ux.md).
...
| R9 | RAG poisoning, stale content, or low-quality source material degrades educational outputs | Safety, operational | 3 | 4 | 12 | In a grounded tutoring system, bad source material becomes a scaled teaching error. | Use approved-content governance through current configuration, Blob Storage, and AI Search integrations; add a dedicated content-svc approval workflow when that boundary is split out. Require teacher or admin-only publishing, source versioning, expiry or recertification dates, and evaluator checks for groundedness against approved material only |
| R10 | Weak provenance and auditability makes it impossible to explain who saw what, why an output was generated, or which model and content were used | Governance, regulatory, operational | 3 | 5 | 15 | Education disputes require traceability. Without provenance, appeals, incident review, and compliance review become brittle. | Persist Foundry agent name/version, model deployment, prompt version, source_ids, evaluator scores, actor identity, and decision state for every high-impact output |
...
| Grounded generation | Use configuration-managed approval metadata, Blob Storage, and AI Search as the approved source of pedagogical truth today; split a dedicated content-svc only when ownership or scale requires it |
