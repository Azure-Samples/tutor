...
const EssayForm = ({ essay, onSuccess }: { essay?: Essay; onSuccess: () => void }) => {
  const [form, setForm] = useState<Essay>(essay || { topic: "", content: "", explanation: "" });
  const [status, setStatus] = useState("");
  const isEdit = !!essay?.id;
  ...
  return (
    <form className="flex flex-col gap-4 w-full max-w-xl">
      ...
      <textarea
        ...
        className="w-full rounded-2xl border-2 border-blue-200 focus:border-yellow-400 focus:ring-2 focus:ring-yellow-200 px-4 py-3 text-lg transition-all duration-200 bg-blue-50 dark:bg-blue-900 placeholder:text-blue-400 focus:bg-white dark:focus:bg-boxdark min-h-[120px]"
      ...
      <textarea
        ...
        className="w-full rounded-2xl border-2 border-green-200 focus:border-cyan-400 focus:ring-2 focus:ring-cyan-200 px-4 py-3 text-lg transition-all duration-200 bg-green-50 dark:bg-green-900 placeholder:text-green-400 focus:bg-white dark:focus:bg-boxdark min-h-[48px] max-h-[320px]"
      ...
      >
        {isEdit ? <FaPen /> : <FaPlus />} {isEdit ? "Update Essay" : "Add Essay"}
      </button>
  ...
  return (
    <div className="w-full p-0">
      ...
      <FormsModal open={showEditModal && !showCreateModal && !showDeleteModal} onClose={closeAllModals} title="Edit Essay">
        <EssayForm essay={selectedEssay!} onSuccess={closeAllModals} />
      </FormsModal>
