...
from app.schemas import Swarm, Evaluator, Essay, Resource
from app.plugins import ImageProcessor, ImageAnswer
...
class AnswerGrader(EvaluatorBase):
    """
    ...
    """
    def __init__(self, grader: Evaluator, kernel: sk.Kernel) -> None:
        """
    ...
    def __prepare(self):
        ...
        rendered_settings = instruction_template.render(instructions=self.grader.metaprompt)
    ...
    def add_tools(self):  # pylint: disable=arguments-differ
        """
        ...
        """
        for tool in [ImageProcessor(), ImageAnswer()]:
    ...
    async def interact(self, essay: Essay, resources: list[Resource], chat: ChatHistory) -> str:  # pylint: disable=arguments-differ
        ...
        rendered_prompt = prompt_template.render(essay=essay, resources=resources)
...
class EvaluatorFactory:
    ...
    def create_graders(self, assembly: Swarm) -> List[AnswerGrader]:
        """
        ...
        """
        graders = []
        for ad in assembly.agents:
            grader = AnswerGrader(grader=ad, kernel=self.__build_kernel())
            grader.agent.description = ad.description or ""
...
class EssayOrchestrator:
    ...
    async def _concurrent_processing(
        ...
        """
        prompt_template = JINJA_ENV.get_template("correct.jinja")
        self.orchestrator = ConcurrentOrchestration[Essay, ChatMessageContent](
            members=[grader.agent for grader in self.graders],
            input_transform=lambda task: ChatMessageContent(
                role=AuthorRole.USER,
                content=prompt_template.render(essay=task, resources=resources)
            ),
            agent_response_callback=self._response_callback,  # type: ignore
        )
        result = await self.orchestrator.invoke(runtime=self.runtime, task=essay)  # type: ignore
    ...
    async def _sequential_processing(
        ...
        """
        prompt_template = JINJA_ENV.get_template("correct.jinja")
        self.orchestrator = SequentialOrchestration[Essay, ChatMessageContent](
            members=[grader.agent for grader in self.graders],
            input_transform=lambda task: ChatMessageContent(
                role=AuthorRole.USER,
                content=prompt_template.render(essay=task, resources=resources)
    ...
    async def _group_processing(
        ...
        self.orchestrator = GroupChatOrchestration[Essay, ChatMessageContent](
            members=[grader.add_tools().agent for grader in self.graders],
            manager=RoundRobinGroupChatManager(max_rounds=max_rounds),
            input_transform=lambda task: ChatMessageContent(
                role=AuthorRole.USER,
                content=prompt_template.render(essay=task, resources=resources)
    ...
    async def invoke(
        ...
        answers = await getattr(self, f"_{strategy}_processing")(essay, resources, *args, **kwargs)
        return answers

    async def fetch_assembly(self, assembly_id: str) -> Swarm:
        """
        ...
        async with CosmosClient(COSMOS_ENDPOINT, DefaultAzureCredential()) as client:
            ...
            return Swarm(**{"id": item["id"], "agents": item["avatars"], "topic_name": item["topic_name"]})
